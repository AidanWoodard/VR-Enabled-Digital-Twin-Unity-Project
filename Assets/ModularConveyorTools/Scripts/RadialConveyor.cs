...
public class RadialConveyor : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		rb.rotation *= Quaternion.AngleAxis (angle * speed * Time.fixedDeltaTime/distance, angleAxis);
		Quaternion newRot = rb.rotation * Quaternion.AngleAxis (-angle * speed * Time.fixedDeltaTime/distance, angleAxis);
	...
	public void ChangeSpeed (float _speed) {
		...
		{
			// For some reason editing prefabs in context can call Start in editor
			return;
		}
#endif
